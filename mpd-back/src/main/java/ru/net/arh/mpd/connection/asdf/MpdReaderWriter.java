...
public class MpdReaderWriter {
    ...
    public List<String> sendCommand(String command) throws IOException {
        ...
        while(true) {
            ...
            result.add(s);
            if (s.startsWith("OK")) {
                log.debug("got result for '{}' command. Result: {}'", command, result.subList(Math.max(result.size()-10, 0), result.size()));
                return result;
            }
            if (s.startsWith("ACK")) {
                throw(ExceptionUtil.logAndGetException("On sending command: '{}' got error '{}'", log, Level.INFO, command.toString(), s.substring(3).trim()));
            }
        }
    }
